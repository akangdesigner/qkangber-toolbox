...
const SPEECH_AUDIT_SYSTEM = `你是狼人殺逐句審訊員。你的工作不是判整局狼坑，而是逐位玩家、逐句建立一份可稽核的證據表，供後續模型使用。所有輸出使用繁體中文。

必須遵守：
- 每一個座位都要獨立審查，不可把數人的發言合併成一段泛論。
- 原句必須來自逐字稿，quote 要短而精確，不可杜撰或改寫成玩家沒說過的話。
- phase 要指出原句所在階段／輪次；若逐字稿無法確定，寫「階段不明」，不可猜測。
- 依戰況與逐字稿先後建立資訊可得性。玩家用投票後才出現的內容解釋更早的票，標為 hard。
- 分別檢查：時間軸、前後立場、預言家站邊、查殺／金水態度、口頭狼坑、實際票型、主動定義未被提出的關係、保人／打人／忽略誰。
- 「主動撇清兩人不是雙狼」只能列為需解釋的關係訊號，不能單獨定狼；必須同時寫出好人解釋。
- 操作快慢、網路延遲不是證據。出局不翻身分；不可用出局結果倒推陣營。
- severity 只能是 hard、medium、weak、none。hard 僅限客觀時間軸不可能、直接自我矛盾或板型不允許；語氣與關係猜測至多 medium。
- 沒有可引用疑點也要輸出該座位，evidence 可為空，不可為了填表製造疑點。
- 證據 id 使用「座位-E序號」，例如「2號-E1」，且全表唯一。
...
只輸出 JSON：
{
  "players":[{
    ...
    "evidence":[{
      ...
      "phase":"第一天警下發言",
      "quote":"逐字稿短原句",
      "finding":"這句具體暴露或矛盾在哪裡",
      "severity":"hard|medium|weak|none",
      "wolfInterpretation":"若是狼，這句的目的或收益",
      "goodInterpretation":"若是好人，最合理的替代解釋"
    }],
    "timelineVerdict":"此人的資訊時間軸結論",
    "consistencyVerdict":"此人的站邊、狼坑與票型一致性結論"
  }]
}`
...
- 使用者會提供「逐人發言審查表」。所有疑點必須引用其中的證據 id；不得跳過審查表，重新憑整體印象創造理由。審查表沒有支持的玩家應降低疑狼度。
...
最終每位玩家的 reason 必須包含逐人發言審查表中的證據 id（例如 2號-E1）；若沒有可用 id，只能寫資訊不足並把疑狼度壓在 59 以下。不得用「可能是隊友」「同票」等無編號泛論補狼坑。
...
請依上述資訊判狼，只回傳 JSON。戰況記錄中的出局與票型是硬資訊，但票型仍須結合投票當時的發言與動機。topWolves 最多 ${board.wolves} 人，只能放有具體證據且 suspicion >= 60 的座位；資訊不足就少列，絕對不要湊數。`
  ...
  // 先讓一個模型逐位、逐句建立共用證據表。後續兩個假設世界與裁判都只能
  // 從這份表取證，避免各模型重新概括逐字稿後各自創造不同的故事。
  const auditRaw = await chatJSON(SPEECH_AUDIT_SYSTEM, userContent, 4200, 0.05)
  const auditParsed = JSON.parse(auditRaw) as { players?: Partial<SpeechAudit>[] }
  const auditBySeat = new Map(
    (Array.isArray(auditParsed.players) ? auditParsed.players : [])
      .filter((item): item is Partial<SpeechAudit> & { seat: string } =>
        typeof item.seat === 'string' && board.seats.some((seat) => seat.id === item.seat))
      .map((item) => [item.seat, item]),
  )
  const speechAudits: SpeechAudit[] = board.seats.map((seat) => {
    const item = auditBySeat.get(seat.id)
    const evidence = (Array.isArray(item?.evidence) ? item.evidence : [])
      .filter((entry) => entry && typeof entry.quote === 'string' && entry.quote.trim())
      .map((entry, index) => ({
        id: typeof entry.id === 'string' && entry.id.trim() ? entry.id : `${seat.id}-E${index + 1}`,
        phase: entry.phase ?? '階段不明',
        quote: entry.quote,
        finding: entry.finding ?? '',
        severity: ['hard', 'medium', 'weak', 'none'].includes(entry.severity)
          ? entry.severity
          : 'weak' as const,
        wolfInterpretation: entry.wolfInterpretation ?? '',
        goodInterpretation: entry.goodInterpretation ?? '',
      }))
    return {
      seat: seat.id,
      evidence,
      timelineVerdict: item?.timelineVerdict ?? '資訊不足，未發現可定位的時間軸矛盾',
      consistencyVerdict: item?.consistencyVerdict ?? '資訊不足，無法判定言行一致性',
    }
  })
  const auditBlock = `【逐人發言審查表（後續推理唯一可引用的發言證據）】\n${JSON.stringify(speechAudits, null, 2)}`
  const groundedContent = `${userContent}\n\n${auditBlock}`

  const claimants = findSeerClaimants(transcript, board.seats.map((s) => s.id)).slice(0, 2)
  if (claimants.length === 2) {
    const worldPrompts = claimants.map((assumedSeer) => `${groundedContent}\n\n【本次強制假設】${assumedSeer}是真預言家，另一位對跳者是假跳。請盤出此世界完整狼坑並誠實列出硬矛盾。`)
  ...
  const raw = await chatJSON(JUDGE_SYSTEM, `${groundedContent}\n\n最終理由只能引用逐人發言審查表中的具體證據 id；沒有證據 id 的玩家不得列為高疑狼。`, 3000, 0.15)
